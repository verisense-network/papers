...
\begin{equation}
  ...
  \begin{aligned}
    \mathbb{E}[b(\omega_i;\theta_\top,\theta_\bot,n)] &= \beta\cdot\sum_{i>\theta_\top\cdot n}{\binom{n}{i}\xi^i(1-\xi)^{n-i}} - \alpha\cdot\sum_{i>\theta_\bot\cdot n}{\binom{n}{n-i}\xi^{(n-i)}(1-\xi)^{i}}
  \end{aligned}
\end{equation}
To find its Nash equilibrium, we take the logarithm of both sides of the equation and find the extreme value of $\xi$ by partial differentiation:
\begin{equation}
  \label{eq:partial}
  \begin{aligned}
    \frac{\partial\mathbb{E}[b(\omega_i;\theta_\top,\theta_\bot,n)]}{\partial\xi} &= \frac{\beta\cdot n\cdot(\xi-\theta_\top)}{\xi\cdot(1-\xi)} - \frac{\alpha\cdot n\cdot(\xi-\theta_\bot)}{\xi\cdot(1-\xi)} = 0\\
    \xi &= \frac{\beta\cdot\theta_\top-\alpha\cdot\theta_\bot}{\beta-\alpha}
  \end{aligned}
\end{equation}
When $\alpha=\beta$, the Nash equilibrium degenerates to $\xi=0$ or $\xi=1$.

\subsubsection{Evaluation of $\theta$}
We do not consider local optimal values for theta, but rather minimize global penalties (in order to prevent voters from colluding as much as possible).
\begin{equation}
  \label{eq:expectation}
  \begin{aligned}
    \mathbb{E}[b(\omega_i;\theta_\top,\theta_\bot,n)\cdot X] &= \beta\cdot\sum_{i>\theta_\top\cdot n}{i\cdot\binom{n}{i}\xi^i(1-\xi)^{n-i}} - \alpha\cdot\sum_{i>\theta_\bot\cdot n}{i\cdot\binom{n}{n-i}\xi^{(n-i)}(1-\xi)^{i}}
  \end{aligned}
\end{equation}
For $\theta_\top$, by taking the partial derivative, we can obtain:
\begin{equation}
  \label{eq:partial}
  \begin{aligned}
    \frac{\partial\mathbb{E}[b(\omega_i;\theta_\top,\theta_\bot,n)\cdot X]}{\partial\theta_\top} &= \beta\cdot\sum_{i>\theta_\top\cdot n}{\binom{n}{i}\xi^i(1-\xi)^{n-i}} - \alpha\cdot\sum_{i>\theta_\bot\cdot n}{\binom{n}{n-i}\xi^{(n-i)}(1-\xi)^{i}} = 0\\
    \theta_\top &= \frac{\beta\cdot\sum_{i>\theta_\top\cdot n}{\binom{n}{i}\xi^i(1-\xi)^{n-i}}}{\alpha\cdot\sum_{i>\theta_\bot\cdot n}{\binom{n}{n-i}\xi^{(n-i)}(1-\xi)^{i}}}
  \end{aligned}
\end{equation}
